...
namespace Assessment2
{
    ...
    public partial class Form_Vehicle : Window
    {
        ...
        private void BtnSave_Click(object sender, RoutedEventArgs e)
        {
            if (id != 0)
            {
                vehicle.EditVehicle(Form_VehicleList.vehicleList, id, txtManufacturer.Text, txtModel.Text, int.Parse(txtYear.Text), txtRegistration.Text, double.Parse(txtOdometer.Text), double.Parse(txtTank.Text));
            }
            else
            {
                vehicle.AddVehicle(Form_VehicleList.vehicleList, txtManufacturer.Text, txtModel.Text, int.Parse(txtYear.Text), txtRegistration.Text, double.Parse(txtOdometer.Text), double.Parse(txtTank.Text));
            }
